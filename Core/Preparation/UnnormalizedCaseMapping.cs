...
namespace Echo.Services.Encoding.Preparation
{
	...
	/// </summary>
	internal class UnnormalizedCaseMapping
	{
		//---------------------------------------------------------------------
		// Data Members
		//---------------------------------------------------------------------
		...
		static UnnormalizedCaseMapping()
		{
		...
		public int[] Mapping(int pKey)
		{
			if (!m_map.ContainsKey(pKey))
				return null;

			// return it
			return (int[])m_map[pKey];
		}
			...
			public void LoadTable(Hashtable pMap)
			{
				...
				try
				{
					for (int index = 0; index < this.Map.Length; index++)
					{
						...
						try
						{
							intKey = int.Parse(parts[0], NumberStyles.HexNumber);
						}
						catch(Exception)
						{
							continue;
						...
						// parse the second parts
						line = StringUtil.Normalize(parts[1]);
						if (null == line)
							continue;
