...
namespace Echo.Services.Encoding.Preparation
{
	...
	/// </summary>
	internal class BidirectionMapping
	{
		//---------------------------------------------------------------------
		// Data Members
		//---------------------------------------------------------------------
		...
		public Direction GetDirection(int pChar)
		{
			Direction	direction = Direction.DIRECTION_NORM;

			// check if it contains the key
			if (!m_map.ContainsKey(pChar))
				return direction;

			try
			{
				direction = (true == (bool)m_map[pChar]) ? Direction.DIRECTION_LEFT : Direction.DIRECTION_RIGHT;
			}
			catch(Exception)
			{
		...
		private class Table
		{
			...
			// Data Members
			//---------------------------------------------------------------------
			private string[]	m_rtable	= null;
			private string[]	m_ltable	= null;
			...
			{
				// load RandAndAL table
				this.LoadTable(m_rtable, false, pMap);

				// local L table
				this.LoadTable(m_ltable, true, pMap);
			}

			//-----------------------------------------------------------------
			// private methods
			//-----------------------------------------------------------------
			private void LoadTable(string[] pTable, bool pFlag, Hashtable pMap)
			{
				bool		bLine	= true;
				int[]		intVal	= null;
				string		line	= null;
				...
				try
				{
					...
					{
						// Next line
						line = StringUtil.Normalize(pTable[index]);
						if (null == line)
							continue;
